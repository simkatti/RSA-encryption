"""This module handles padding for messages to be encrypted and unpadding for decryption.
...
class Padder:
    def __init__(self):
        ...
        self.hll_length = len(self.lhash)
        self.k = None
        self.m_len = None
    ...
    def perform_padding(self, n, m):
        ...
        ps = self.generate_padding_string(self.k, self.m_len, self.hll_length)
        datablock = self.lhash + ps + b'\x01' + m.encode('utf-8')
        seed = self.generate_seed(self.hll_length)
        db_mask = self.mask_generation_function(seed, self.k - self.hll_length - 1)
        masked_db = self.xor(datablock, db_mask)
        seed_mask = self.mask_generation_function(masked_db, self.hll_length)
        masked_seed = self.xor(seed, seed_mask)

        return b'\x00' + masked_seed + masked_db

    def mask_generation_function(self, seed, l):
        if l > (self.hll_length << 32):
    ...
    def generate_padding_string(self, k, m_len, h_len):  # k-mLen - 2 *hLen-2
    ...
    def reverse_padding(self, n, em):
        self.k = len(n.to_bytes((n.bit_length() + 7) // 8, "big"))
        # split em into 0x00, maskedSeed length of hLen AND maskedDb
        null_byte, masked_seed, masked_db = em[0:1], em[1:self.hll_length +1], em[self.hll_length + 1:]
        seed_mask = self.mask_generation_function(masked_db, self.hll_length)
        seed = self.xor(masked_seed, seed_mask)
        db_mask = self.mask_generation_function(seed, self.k - self.hll_length - 1)
        datablock = self.xor(masked_db, db_mask)
        lhash = datablock[:self.hll_length]
        sliced_db = datablock[self.hll_length:]
        separating_byte = sliced_db.index(b'\x01')
        ps = sliced_db[:separating_byte]
        m = sliced_db[separating_byte + 1:]

        # padding is successful
        if lhash == self.lhash and ps == b'\x00' * len(ps) and \
            sliced_db == ps + b'\x01' + m and em[0] == 0x00:
            return m
