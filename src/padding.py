...
class Padder:
    ...
    def perform_padding(self, n, m):
        ...
        db_mask = self.mask_generation_function(
            seed, self.k - self.lhash_len - 1)
    ...
    def mask_generation_function(self, seed, l):
        """For generating a mask of given length"""
    ...
    def generate_padding_string(self, k, m_len, h_len):
        """Byte string of 0x00 * k - mLen - 2* hLen -2"""
        bytecount = k - m_len - 2 * h_len - 2
        return b'\x00' * bytecount

    def generate_seed(self, length):
        """Generates a random seed. A single byte (8-bits) can represent 256 numbers"""
        return bytes(random.randint(0, 255) for _ in range(length))

    def split_datablock(self, datablock):
        """Datablock is split in order to reverse the padding when decrypting"""
    ...
    def reverse_padding(self, n, em):
        """Reversing the padding steps to remove padding"""
        self.k = len(n.to_bytes((n.bit_length() + 7) // 8, "big"))
        null_byte, masked_seed, masked_db = em[0:1], em[1:self.lhash_len +
                                                        1], em[self.lhash_len + 1:]
        seed_mask = self.mask_generation_function(masked_db, self.lhash_len)
